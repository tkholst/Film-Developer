...
short temperatureTolerence = 5; // set tolerance +-1 degree centigrade of the temperature written in txt file
short dataExecutionCounter = 0;
short speed(350); //sets agitation speed, higher # slows down, set between 200 and 1000. 
bool dir = true;
float RawHigh = 99.62;
float RawLow = 0.25;
float ReferenceHigh = 99.7;
...
//  float CorrectedValue = (((RawValue - RawLow) * ReferenceRange) / RawRange) + ReferenceLow;
  float CorrectedTemp = (((temperature - RawLow) * ReferenceRange) / RawRange) + ReferenceLow;  
//  temperature = 25;
...
void startingGUI() {
  float CorrectedTemp = (((temperature - RawLow) * ReferenceRange) / RawRange) + ReferenceLow;  
  float tempCheck=CorrectedTemp-dataToexecute[1]; //Houston, it seems like we have a problem here with tempCheck
  if (tempCheck<0){
    tempCheck=-1*tempCheck;
    }
    tempCheck=.5; //uncomment to override tempchk //if you do, you will have problems as long as logo is activated
  ...
  for (int rl = 2; rl <= dataExecutionCounter; rl++) {
    ...
    for (int rk = 2; rk <= dataExecutionCounter; rk++) {
      if (rk <= rl) {
        TFTscreen.stroke(255, 255, 0); //aktiv prosesstid
        TFTscreen.print("  ");
        TFTscreen.print(dataToexecute[rk]);
      }
      else {
        TFTscreen.stroke(155, 155, 155); //ventende prosesstid
        TFTscreen.print("  ");
        TFTscreen.print(dataToexecute[rk]);
      }
      TFTscreen.print(" s. ");
  ...
  TFTscreen.setTextSize(1);
  TFTscreen.rect( 0, (TFTscreen.height() / 2) - 45, TFTscreen.width(), 90);
  TFTscreen.setCursor(0, (TFTscreen.height() / 2) - 25);
  TFTscreen.println("  PRESS ANY KEY");
  TFTscreen.print("  TO CONTINUE!");
  BS = Button();
  resetFunc();
  }
}
...
//      delay(1000);
  ...
  delay(1000); //set to 1000 to beep for 1 second
  digitalWrite(buzzerPin, LOW);
  resetStepper();
  delay(3000); //can be set to 5000
...
void fileGUI() {
  ...
  TFTscreen.setCursor(0, 22);
  TFTscreen.setTextSize(1);
  TFTscreen.stroke(155, 155, 155); //tekstfarge filliste
  for (int i = 1; i <= maximumFileOnScreen; i++) {
    if (i == currentPosition) {
      TFTscreen.stroke(255, 255, 0); //tekstfarge markert fil
      TFTscreen.print("   ");
